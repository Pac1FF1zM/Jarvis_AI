# Обучение Jarvis Gesture Core на IPN Hand

Это отдельный ML-контур для распознавания жестов рукой через веб-камеру.
Он использует только RGB-видео IPN Hand и нейросети, созданные в этом
репозитории с нулевой инициализацией. Нет Hugging Face, MediaPipe, готовых
весов, embeddings или скачиваемого backbone.

## Что скачать

С официальной страницы IPN Hand скачайте только:

- **Videos** — пять файлов `videos01.tgz`…`videos05.tgz`, около 4.6 ГБ;
  несмотря на название раздела на сайте, внутри официальных архивов находятся
  200 файлов `.avi`, которые импортёр читает напрямую;
- **Annotations** — небольшая папка с `Annot_TrainList.txt`,
  `Annot_TestList.txt` и `classIdx.txt`.

Не нужны `Video frames`, `Optical flow frames` и `Hand segmentation frames`.
Не извлекайте MP4 в JPEG: набор из кадров заметно больше исходных видео.

IPN Hand содержит 13 жестов и `D0X` (обычные движения без команды). Этот
последний класс критичен: он измеряет ложные срабатывания, а не только
красивую accuracy на жестах.

Источник и лицензия: <https://gibranbenitez.github.io/IPN_Hand/> (CC BY 4.0).

## Подготовка на RTX 3090 Ti

Перенесите **весь актуальный проект Jarvis** на компьютер друга, затем в
PowerShell из корня проекта выполните один раз:

```powershell
Set-ExecutionPolicy -Scope Process Bypass
.\training_workspace\SETUP_RTX3090.ps1 -PythonVersion "-3.12" -CudaWheel "cu128"
.\.venv-training\Scripts\python.exe -m pip install opencv-python-headless
```

Последняя команда добавляет только декодер локальных MP4. Она не скачивает
никаких моделей.

## Распаковка и импорт

Создайте, например, папку `D:\JarvisGesture\ipn\videos`. Распакуйте туда
каждый `.tgz`, чтобы внутри в итоге лежали `.avi`. Разметку скачайте или распакуйте в
`D:\JarvisGesture\ipn\annotations`.

Создайте проверенный manifest:

```powershell
.\training_workspace\IMPORT_IPN_HAND.ps1 `
  -Videos "D:\JarvisGesture\ipn\videos" `
  -Annotations "D:\JarvisGesture\ipn\annotations"
```

Скрипт напрямую читает официальный Google Drive формат `.txt`, сопоставляет
все сегменты с AVI/MP4 и отказывается создавать manifest при неизвестной метке
или отсутствующем видео. Официальный test остаётся закрытым test, а внутренняя
validation детерминированно выделяется из train по людям — один человек не
попадает одновременно в эти части. В `training_workspace/gesture_data/`
появится только маленький JSONL; сами видео в Git не добавляются.

## Проверка до обучения

Если IPN ещё не скачан на этом компьютере, можно проверить декодирование AVI,
CUDA и полный цикл трёх архитектур на маленьком синтетическом наборе:

```powershell
.\training_workspace\START_GESTURE_TRAINING.ps1 -Smoke
```

Smoke-режим запускает каждую архитектуру на 3 эпохи, но намеренно не создаёт
разрешённые к использованию веса. Он проверяет код и железо, а не качество
распознавания жестов. Обычный запуск по-прежнему требует настоящий IPN.

```powershell
.\training_workspace\START_GESTURE_TRAINING.ps1 -CheckOnly
```

Ожидается JSON с CUDA, количеством сегментов, количеством отдельных видео в
`train` / `validation` / `test` и распределением меток. Если тут ошибка —
обучение запускать не надо: сначала исправляется путь или распаковка.

## Обучение

```powershell
.\training_workspace\START_GESTURE_TRAINING.ps1
```

Будут обучены три разные модели с нуля:

1. `tiny_3d_cnn` — сразу учит пространственно-временной паттерн;
2. `cnn_bigru` — свёрточный анализ каждого кадра + двунаправленный GRU для
   траектории движения;
3. `cnn_temporal_transformer` — свёрточный анализ кадров + temporal attention.

На Windows параметр `num_workers` намеренно равен `0`: официальный IPN
содержит AVI, а OpenCV может нестабильно читать их из нескольких процессов
DataLoader одновременно. Декодирование выполняется в основном процессе,
при единичном сбое контейнер открывается заново, а вычисления нейросети и AMP
по-прежнему работают на CUDA. Это безопаснее, чем потерять многочасовой запуск
из-за случайного сбоя декодера или незаметно подменить размеченный кадр.
Если AVI seek-index остаётся нечитаемым после всех попыток OpenCV, PyAV
последовательно читает тот же поток через независимую FFmpeg-привязку. Этот
медленный путь используется только для проблемных клипов.

Сравнение архитектур выполняется только по `validation`. Официальный `test`
раскрывается один раз после выбора победителя. В результатах важны не только
`macro_f1`, но и `no_gesture_recall` и `false_trigger_rate`.

Веса экспортируются в `training_workspace/gesture_export/` только если
выбранная модель проходит все safety-gates в `gesture_config.yaml`. Если
экспорта нет — это честный результат: модель ещё не готова управлять Jarvis.

## После baseline

IPN не включает жесты именно Jarvis вроде «ладонь = стоп». Их добавим вторым
этапом небольшой собственной выборкой; она должна быть отделена по людям и
освещению от финального теста. Пока нельзя подмешивать тест IPN или его видео
в тренировку ради улучшения метрики.

## Безопасный runtime после обучения

В проекте уже есть выключенный по умолчанию модуль `gesture` в `config.yaml`.
Он загружает только checkpoint с полем `pretrained: false`, включает камеру
только после события `gesture_mode_requested`, а предложение жеста выпускает
только после confidence-порога, трёх одинаковых временных окон и cooldown.

После реального тестирования камеры можно включить в `config.yaml` одновременно
`modules.gesture.enabled: true` и `execution_enabled: true`. Тогда голосовая
команда «включи режим жестов» включает камеру, а «выключи режим жестов» —
выключает её. Во время режима wake word и обычные голосовые команды продолжают
работать: жестовый вход не блокирует основной разговорный Hub Jarvis.

Отдельный `GestureActionBridge` пропускает только проверенный безопасный
allow-list: громкость, предыдущий/следующий трек, пауза/продолжение и масштаб
активного браузера. Указатели, жесты подтверждения и любые действия с файлами,
окнами или приложениями намеренно не исполняются. При `execution_enabled: false`
модель остаётся в proposal-only режиме, поэтому её можно тестировать на реальной
камере без системных побочных эффектов.
