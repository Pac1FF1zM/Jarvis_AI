...
Даже тогда модуль публикует лишь `gesture_action_ready`. В текущей версии на
это событие **нет исполнителя**: ни клавиши, ни приложения, ни системные
настройки жестом пока не меняются. Это осознанная защита. После реального
тестирования камеры отдельным этапом добавим строго ограниченный bridge к
разрешённым действиям Jarvis.
