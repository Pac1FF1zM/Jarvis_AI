...
Текущий runtime включён как ограниченный real-camera тест. Он включает камеру
только по запросу, а предложение жеста выпускает после confidence-порога,
трёх одинаковых временных окон, cooldown и возврата в `D0X`.

Голосом доступны запуск, выключение, пауза, продолжение и проверка состояния;
`Ctrl+Alt+/` переключает режим, пока работает Jarvis. Пауза оставляет камеру и
UI активными, но блокирует действия. Закрытие окна скрывает только UI. После
перезапуска режим всегда выключен. Голосовые команды работают параллельно.

Отдельный `GestureActionBridge` пропускает только G01-G06: паузу/воспроизведение,
mute, громкость вверх/вниз на два шага и предыдущий/следующий трек. Действия
выполняются без TTS. Все остальные классы, указатели, подтверждения, браузер,
файлы, окна и приложения заблокированы. JSONL-телеметрия хранится без видео и
кадров не более 30 дней и 100 МБ. При `execution_enabled: false` модель остаётся
в proposal-only режиме.
