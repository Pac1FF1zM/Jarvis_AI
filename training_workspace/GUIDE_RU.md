...
Если широкий phase-one уже завершён, но прежний scalar ranking выбрал
неудачных финалистов, не повторяйте все trial. Feasible-first режим
переиспользует готовый отчёт, сначала выбирает кандидатов без hard-gate
нарушений и обучает только их на confirmation seeds:

```powershell
.\training_workspace\START_TRAINING.ps1 -ConfirmFromRun "C:\полный\путь\report.json"
```

Для custom-validation latency используется медиана трёх независимых p95
замеров. Порог не ослабляется; повторение только уменьшает влияние планировщика
Windows и фоновых процессов.
