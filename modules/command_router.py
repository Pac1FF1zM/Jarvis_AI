...
def _resolve_application_phrase(value: str):
    ...
    cleaned = value.strip(" ,.!?:;-")
    cleaned = re.sub(
        r"^(?:(?:стандартн\w*|обычн\w*)\s+)?(?:окно|приложение|программу|программа)\s+", "", cleaned
    )
    cleaned = re.sub(r"^(?:стандартн\w*|обычн\w*)\s+", "", cleaned)
    cleaned = re.sub(
...
def route_explicit_command(
    ...
    value = re.sub(r"^(?:можешь|мог бы|могла бы)(?: пожалуйста)?\s+", "", value)
    value = re.sub(r"^открыть\b", "открой", value)
    value = re.sub(r"^запустить\b", "запусти", value)
    value = re.sub(
        rf"^({_COMMAND_START})\s*,?\s*пожалуйста\s*,?\s+", r"\1 ", value
    )
    if re.fullmatch(
    ...
    gesture_target = (
        ...
        r"управление\s+с\s+помощью\s+жестов|распознавание\s+жестов|жесты)"
    )
    gesture_patterns = (
        (
        ...
        (
            ...
            rf"проверь\s+(?:работает|включен|активен)\s+ли\s+(?:сейчас\s+)?{gesture_target}|"
    ...
    if re.fullmatch(
        ...
        return RoutedAction("get_current_time")
    if re.fullmatch(r"(?:покажи|назови|перечисли|какие) (?:доступные |установленные )?(?:приложения|программы)(?: (?:которые )?ты (?:можешь|умеешь) открывать)?", value):
