...
- Долговременная память хранит только явно запрошенные факты в локальной
  SQLite-базе и разделяет их по профилям. Команды «запомни…», «что ты обо мне
  знаешь?», «как меня зовут?» и «забудь…» работают без Ollama; полная очистка
  требует подтверждения. В свободном диалоге в локальную Ollama передаётся
  только ограниченное число фактов, помеченных как данные, а не инструкции.
- NLU понимает команды «включи/выключи режим жестов» и управляет независимым
  webcam-модулем через EventBus. Текущий IPN Hand checkpoint не прошёл
...
```text
Запомни, что меня зовут Фирдавс.
