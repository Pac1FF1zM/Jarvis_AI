...
- Frozen TSN-ResNet18 baseline показывает macro-F1 0.5858 на официальном
  subject-disjoint test. Для real-camera проверки разрешены только обратимые
  действия: `G01` пауза/воспроизведение, `G02` mute, `G03/G04` громкость на
  два шага, `G05/G06` предыдущий/следующий трек. Жесты выполняются без TTS,
  требуют 80% × 3 окна и возврата в `D0X`. Остальные классы заблокированы.
...
- новая контролируемая голосовая приёмка после выбранной VAD-настройки
  (исторические реальные сессии уже измерены);
